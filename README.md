...
In this document, we'll spin up our own distributed network of databases across multiple regions of Google Cloud and then apply the ETL ([Extract-Transform-Load](https://en.wikipedia.org/wiki/Extract,_transform,_load)) pattern to the data stored in each location with Bacalhau.

### The Problem

Why are we doing this? Well, modern infrastructure generates a lot of data in many different places. Keeping tabs on where that data is and accessing it when you want to gain some insight can be both logistically complex and expensive.

Typically, there are two approaches:

1. You can aggregate data from all your data sources in a centralised location and perform queries on that new, larger datastore.
...
Both approaches present challenges that aren't easy to overcome. When aggregating all of your data, multiple factors can make the approach untenable: the cost of bandwidth, the cost of centralised storage, the time required to execute queries, and more.
...
With Bacalhau, we can create a common interface for querying and filtering data at each source point. Then, we can aggregate a reduced set of key data points for further analysis at a central location.

This process tackles the problems presented by both typical approaches. Instead of incurring fees transferring all of your data from your different sources, we extract only the data that we consider useful for any given application. 
...
For this example project, we will be aggregating our filtered data in a [MongoDB Atlas](https://www.mongodb.com/atlas/database) database. We're using Atlas for this example, as it has an abundant free-tier that we can use to experiment and explore with. Given that, we'll be coupling it with [MongoDB Community Edition](https://www.mongodb.com/try/download/community) in each of our deployed instances, we can keep the exact data structure and interfaces to talk to both our centralised database, as well as those at the edge.
...
Once you've finished the sign-up process, you'll be taken to your new account's "Overview" page. Click the "Create a deployment" button (highlighted in red).
...
5. Select a region for the instance to be deployed in.
    - You can choose any region you like, and it doesn't matter which one you pick at this point, but try to pick something closest to your geographical location to reduce latency later when we're interacting with our stored data.
...
We don't need to do all of this quite at the moment, but copy the username and password presented in the form as the default user, and save them somewhere secure on your system. We'll need this information later. Then click the "Create Database User" button. 
...
To the top-right of that table, click the "ADD IP ADDRESS" button (highlighted in blue). In the modal that appears, click "ALLOW ACCESS FROM ANYWHERE" and then "Confirm."
...
First, you need to install the Google Cloud SDK. Depending on your platform, you will need to install it from a package manager or directly compile it. You can find the instructions to do so [here](https://cloud.google.com/sdk/docs/install).
...
For this project, we're using Terraform to orchestrate our infrastructure. If you haven't done so, now would be a good time to install Terraform on your local system. You can find instructions on that [here](https://learn.hashicorp.com/tutorials/terraform/install-cli).
...
3. `scripts` directory
    - The folder contains some Python scripts which will generate reports every 1/3 of a second on the status and utilisation of our servers. We'll be filtering and extracting these reports with the help of Bacalhau later on.
...
Next, we generate the 'plan' for our deployment. This will create a `plan.out` file in our `terraform` directory which will describe all of the steps and resources needed to deploy our infrastructure. Run the following command to generate the plan.
...
After a few minutes, we're ready to fire off a job to each of our nodes to filter our logs for instances where more than 50% of the system's CPU has been utilised at a given time. We then forward only those records to Atlas for further analysis.
...
Now that our jobs are running, data is being generated, stored, transformed, and extracted from each of our locations.
...
If all has gone well, you should see a database filled with records from each of our locations, showing only the information that we've deemed useful in this circumstance (that is, instances that are utilising greater than 50% of our CPU capacity).
...
With that, we have an ETL pipeline that can extract, transform, and load our data from any number of data sources across the internet and aggregate them into a MongoDB Atlas Database!

But there's more we can do! 

If you'd like to learn more about the best practices for setting up a Private Cluster with Bacalhau and how to secure it [we have a blog post on that](https://blog.bacalhau.org/p/setting-up-a-bacalhau-cluster-tips).
